...
static std::shared_ptr<Tensor> BuildCausalMaskLike(const std::shared_ptr<Tensor>& /*scores*/) {
  return nullptr; 
}
    ...
    bool enable_gqa)                            
{
  ...
  if (attn_mask) {
    ...
    if (causal) scores = scores + causal;
  }
    ...
    bool enable_gqa)                            
{
  ...
  auto ones = std::make_shared<Tensor>(std::vector<int64_t>{1, 1, Tk, 1},
                                       P->Dtype(), P->GetDevice());
