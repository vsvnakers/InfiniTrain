...
__global__ void FlashAttnForwardKernel(
    T* __restrict__ y,                      // (B,H,Tq,D)
    const T* __restrict__ q,                // (B,H,Tq,D)
    const T* __restrict__ k,                // (B,H,Tk,D)
    const T* __restrict__ v,                // (B,H,Tk,D)
    const float* __restrict__ add_mask,     
    int B, int H, int Tq, int Tk, int D,
    float scale, bool causal)
{
    ...
    for (int d = tid; d < D; d += BLOCK_THREADS) {
        y_row[d] = common::cuda::Cast<T>(0.0f);
    }
    ...
    for (int tk = 0; tk < Tk; ++tk) {
        ...
        if (tid == 0) { m = m_new; l = l_new; }
        __syncthreads();
    }
}

template <int BLOCK_THREADS, typename T>
void LaunchFlashAttnForward(
    ...
    bool enable_gqa)
{
    ...
    switch (q->Dtype()) {
        DISPATCH_CASE(
            WRAP(LaunchFlashAttnForward<256, float>(
                y, q, k, v, attn_mask, is_causal, dropout_p, scale_has, scale_val, enable_gqa);),
            DataType::kFLOAT32)
        DISPATCH_CASE(
            WRAP(LaunchFlashAttnForward<256, nv_bfloat16>(
                y, q, k, v, attn_mask, is_causal, dropout_p, scale_has, scale_val, enable_gqa);),
            DataType::kBFLOAT16)
    default:
...
template <int BLOCK_THREADS, typename T>
__global__ void RowwiseMaxKernel(const T* __restrict__ x, float* __restrict__ row_max,
                                 int B, int H, int Tq, int Tk) {
    int b  = blockIdx.x;
    int h  = blockIdx.y;
    int tq = blockIdx.z;
...
template <int BLOCK_THREADS, typename T>
__global__ void NormalizeKernel(T* __restrict__ P, const T* __restrict__ scores,
...
template <typename T>
__global__ void AddCausalNegInfKernel(T* __restrict__ s, int B, int H, int Tq, int Tk) {
    int b  = blockIdx.x;
    int h  = blockIdx.y;
    int tq = blockIdx.z;
    int64_t base = (((int64_t)b*H + h)*Tq + tq) * (int64_t)Tk;
...
std::vector<std::shared_ptr<Tensor>> ScaledDotProductAttentionBackward(
    const std::shared_ptr<Tensor>& dY,          // (B,H,Tq,D)
    ...
    if (is_causal) {
        ...
        switch (scores->Dtype()) {
            DISPATCH_CASE(
                WRAP(AddCausalNegInfKernel<float><<<grid, block, 0, dev->Stream()>>>(
                        static_cast<float*>(scores->DataPtr()), B, H, Tq, Tk);),
                DataType::kFLOAT32)
            DISPATCH_CASE(
                WRAP(AddCausalNegInfKernel<nv_bfloat16><<<grid, block, 0, dev->Stream()>>>(
    ...
    auto row_sumexp = std::make_shared<Tensor>(std::vector<int64_t>{B, H, Tq, 1}, DataType::kFLOAT32, scores->GetDevice());
    ...
    switch (scores->Dtype()) {
        DISPATCH_CASE(
            WRAP(RowwiseMaxKernel<256, float>      <<<grid, block, 0, dev2->Stream()>>>(static_cast<const float*>(scores->DataPtr()),     static_cast<float*>(row_max->DataPtr()), B,H,Tq,Tk);
                 RowwiseSumExpKernel<256, float>   <<<grid, block, 0, dev2->Stream()>>>(static_cast<const float*>(scores->DataPtr()),     static_cast<const float*>(row_max->DataPtr()), static_cast<float*>(row_sumexp->DataPtr()), B,H,Tq,Tk);
                 NormalizeKernel<256, float>       <<<grid, block, 0, dev2->Stream()>>>(static_cast<float*>(P->DataPtr()),                 static_cast<const float*>(scores->DataPtr()),  static_cast<const float*>(row_max->DataPtr()), static_cast<const float*>(row_sumexp->DataPtr()), B,H,Tq,Tk);),
            DataType::kFLOAT32)
        DISPATCH_CASE(
            WRAP(RowwiseMaxKernel<256, nv_bfloat16><<<grid, block, 0, dev2->Stream()>>>(static_cast<const nv_bfloat16*>(scores->DataPtr()), static_cast<float*>(row_max->DataPtr()), B,H,Tq,Tk);
                 RowwiseSumExpKernel<256, nv_bfloat16><<<grid, block, 0, dev2->Stream()>>>(static_cast<const nv_bfloat16*>(scores->DataPtr()), static_cast<const float*>(row_max->DataPtr()), static_cast<float*>(row_sumexp->DataPtr()), B,H,Tq,Tk);
                 NormalizeKernel<256, nv_bfloat16> <<<grid, block, 0, dev2->Stream()>>>(static_cast<nv_bfloat16*>(P->DataPtr()),            static_cast<const nv_bfloat16*>(scores->DataPtr()), static_cast<const float*>(row_max->DataPtr()), static_cast<const float*>(row_sumexp->DataPtr()), B,H,Tq,Tk);),
            DataType::kBFLOAT16)
        default:
