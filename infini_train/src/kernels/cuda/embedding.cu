...
__global__ void EmbeddingForwardKernel(const uint16_t *input, float *output, const float *wte, const float *wpe,
...
std::shared_ptr<Tensor> EmbeddingForward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &wte,
...
EmbeddingBackward(const std::shared_ptr<Tensor> &input, const std::shared_ptr<Tensor> &wte,
